...
<div class="w3-card-4 w3-section">
  <div class="w3-container w3-highway-red" style="margin-top:100px">
    <h2>Nuestros tipos de vinos</h2>
  </div>
  <form action="vinosColores.php" method="post">
  <div class="w3-container w3-image foto" style="width: 100%;margin-left: 0px padding:10px">
  <select name='tipo'> <!-- Esto es lo que se envia al script de borrado para que sepa que esta seleccionado en el select -->
      ...
      <?php
        ...
        $consulta = "SELECT tipo FROM vinos GROUP BY tipo";
        $resultado = mysqli_query($conn, $consulta) or die(mysqli_error($conn));

        foreach ($resultado as $opciones) {
          # code...
          echo "<option value=" . $opciones['tipo'] . ">" .$opciones['tipo'] . "</option>"; 
        }
      
        ?>
  </select>
  <p>
    <input type="submit" style="margin-left: 0px" class="w3-btn w3-highway-red" value="Buscar" style="margin-left:20px;margin-bottom:5px "></input></p>
